...
# Visual Web Headers (Matching your preferred style)
...
with col1:
    # Dropdown choice selection field (Alphabetically sorted)
    selected_station = st.selectbox(
        "Select Weather Station:",
        options=sorted(list(ZIM_STATIONS.keys()))
    )
    
    # Extracting using your exact backend keys
...
# Calculation execution block trigger matching your submission button text
if st.button("Calculate Atmospheric Values", type="primary", use_container_width=True):
    if db_input and wb_input:
        try:
            ...
            # Success message matching your green window layout
            st.success("Calculations Complete")

            # Displays your 3 metrics using the exact dictionary results mapping
